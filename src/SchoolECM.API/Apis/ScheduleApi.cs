...
public static class ScheduleApi
{
    ...
    {
        var api = app.MapGroup("api/schedule").HasApiVersion(1.0);

        /*api.MapPut("/cancel", CancelOrderAsync);
        api.MapPut("/ship", ShipOrderAsync);*/
        api.MapGet("{scheduleId:int}", GetScheduleAsync);
        api.MapGet("/", GetSchedulesByUserAsync);
        api.MapGet("/disciplines", GetAllDisciplinesByUserAsync);
        api.MapGet("/groups/{disciplineId:int}", GetAllGroupsByUserAsync);
    ...
    /*public static async Task<Results<Ok, BadRequest<string>, ProblemHttpResult>> CancelOrderAsync(
        ...
        [AsParameters] SchoolECMServices services)
    {
    ...
    public static async Task<Results<Ok<ScheduleDetails>, NotFound>> GetScheduleAsync(int scheduleId, [AsParameters] SchoolECMServices services)
    {
        try
        {
            var schedule = await services.Queries.GetScheduleAsync(scheduleId);
    ...
    public static async Task<Ok<IEnumerable<ScheduleSummary>>> GetSchedulesByUserAsync([AsParameters] SchoolECMServices services)
    {
        var userId = services.IdentityService.GetUserIdentity();
        var schedules = await services.Queries.GetSchedulesFromUserAsync(userId);
        return TypedResults.Ok(schedules);
    }
    public static async Task<Ok<IEnumerable<DisciplineViewModel>>> GetAllDisciplinesByUserAsync([AsParameters] SchoolECMServices services)
    {
        var userId = services.IdentityService.GetUserIdentity();
        var disciplines = await services.Queries.GetAllDisciplinesFromUserAsync(userId);
        return TypedResults.Ok(disciplines);
    }

    public static async Task<Ok<IEnumerable<GroupViewModel>>> GetAllGroupsByUserAsync(int disciplineId, [AsParameters] SchoolECMServices services)
    {
        var userId = services.IdentityService.GetUserIdentity();
        var disciplines = await services.Queries.GetAllGroupsFromUserAsync(userId, disciplineId);
        return TypedResults.Ok(disciplines);
    }

    /*public static async Task<Ok<IEnumerable<CardType>>> GetCardTypesAsync(IScheduleQueries orderQueries)
    {
        var cardTypes = await orderQueries.GetCardTypesAsync();
        return TypedResults.Ok(cardTypes);
    }*/

    /*public static async Task<OrderDraftDTO> CreateOrderDraftAsync(CreateOrderDraftCommand command, [AsParameters] SchoolECMServices services)
    {
        services.Logger.LogInformation(
            "Sending command: {CommandName} - {IdProperty}: {CommandId} ({@Command})",
