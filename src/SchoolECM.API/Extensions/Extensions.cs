internal static class Extensions
{
    public static void AddApplicationServices(this IHostApplicationBuilder builder)
    {
        ...
        // The DbContext of type 'SchoolECMContext' cannot be pooled because it does not have a public constructor accepting a single parameter of type DbContextOptions or has more than one constructor.
        services.AddDbContext<SchoolECMContext>(options =>
        {
            options.UseNpgsql(builder.Configuration.GetConnectionString("orderingdb"));
        });
        builder.EnrichNpgsqlDbContext<SchoolECMContext>();

        services.AddMigration<SchoolECMContext, SchoolECMContextSeed>();

        // Add the integration services that consume the DbContext
        services.AddTransient<IIntegrationEventLogService, IntegrationEventLogService<SchoolECMContext>>();
/*
        ...
        services.AddScoped<IGradesQueries, GradesQueries>();/*
